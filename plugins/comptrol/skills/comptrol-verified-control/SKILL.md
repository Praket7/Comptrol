...
If using ChatGPT web, follow `CHATGPT_SETUP.md` in the plugin root to connect the local stdio server through OpenAI Secure MCP Tunnel. Do not tell the user that ChatGPT can reach a `127.0.0.1` endpoint directly. For local MCP clients, server startup and any write approval remain user-controlled.
