...
The release workflow prepares native builds on Linux, macOS, and Windows. After native CI passes, it publishes the `comptrolling` npm launcher and attaches a Homebrew formula generated from the verified macOS archive. The manually published initial Homebrew artifact is Apple Silicon only. Signing, attestations, and a public cross platform benchmark remain future release work.
