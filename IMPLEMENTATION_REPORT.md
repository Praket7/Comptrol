...
The `v0.1.25` GitHub release workflow completed successfully across all five native targets, published release assets and the npm package, and the package became visible on the public registry after npm processing. The next release target includes target-scoped visual evidence for dynamic-site recovery, direct Windows UIA source, and the existing bounded post-publish registry visibility verification, event-driven history verification, target-list caching, WSL-versus-Windows binary diagnostics, and isolated adapter runtime validation.
...
The `comptrolling@0.1.24` npm package is publicly visible and was verified after registry propagation. The `v0.1.25` package adds target-scoped screenshot digests for visual recovery, alongside the isolated adapter SDK and host, four first party adapter runners, event driven compact browser URL verification, adapter scaffolding and manifest validation, adapter conformance smoke tests, and richer benchmark resource and false positive fields. The portable local plugin package and repo marketplace are fixture validated, but ChatGPT developer mode registration and live ChatGPT installation remain user side acceptance steps. Detached signing remains opt in because no operator signing identity was supplied. The fast verified work is being released from `feat/fast-verified-execution-v3`; live application acceptance still depends on the relevant application and user granted bridge consent. GitHub Dependency Graph is disabled for this repository, so dependency review reports a warning while RustSec `cargo-audit` remains enforced by the security workflow.
