...
The `v0.1.26` GitHub release workflow is the current release target. It includes target-scoped visual evidence for dynamic-site recovery, direct Windows UIA source, honest staged upload verification, bounded post-publish registry visibility verification, event-driven history verification, target-list caching, WSL-versus-Windows binary diagnostics, and isolated adapter runtime validation. The final release statement is updated only after the Windows workflow and npm registry checks complete.
...
The `comptrolling@0.1.25` npm package was publicly visible after registry propagation. The pending `0.1.26` package adds direct Windows UIA source and staged generic upload semantics on top of target-scoped screenshot digests, the isolated adapter SDK and host, four first party adapter runners, event driven browser verification, adapter scaffolding and manifest validation, adapter conformance smoke tests, and richer benchmark resource and false positive fields. The portable local plugin package and repo marketplace are fixture validated, but ChatGPT developer mode registration and live ChatGPT installation remain user side acceptance steps. Detached signing remains opt in because no operator signing identity was supplied. The fast verified work is being released from `feat/fast-verified-execution-v3`; live application acceptance still depends on the relevant application and user granted bridge consent. GitHub Dependency Graph is disabled for this repository, so dependency review reports a warning while RustSec `cargo-audit` remains enforced by the security workflow.
