...
The loopback HTTP preview validates the origin and binds only to localhost. It also serves a read only diagnostics dashboard. The standard compatibility path is MCP stdio. An opt in `serve-mtls` transport binds a configured address, requires a server certificate and key plus a client CA, completes mutual TLS before parsing HTTP, and reuses the same bounded MCP/session handler.
...
The npm package dry run and launcher restart conformance passed. Browser fixture conformance passed with an explicit transport assertion of one page websocket and one browser websocket across repeated target-bound CDP operations. Upload and download now use those same cached browser/page sessions. Screenshot capture can return bounded pixels on request together with a capture token and viewport geometry; coordinate clicks reject stale tokens or out-of-bounds coordinates before dispatch.
...
Portable CDP closed-group restoration remains intentionally unsupported because Chrome exposes no portable closed-group history through DevTools. A local Manifest V3 Chrome extension now provides the supported profile-native implementation through the `sessions` and `tabGroups` APIs, with unique matching and post-restore membership verification. Native Linux/macOS live fixture validation and live VS Code, LibreOffice, OBS, and Blender acceptance remain pending on hosts with working accessibility/automation bridges. Streamable HTTP has concurrent long lived GET streams, bounded event history, `Last-Event-ID` replay, persistent session state, safe restart loading, session deletion, and a connection ceiling. The browser session cache is shared by normal CDP calls, upload, and download. Generic CDP upload still reports only the selected-file stage as unverified until a site or application adapter proves transfer acceptance. Event sequencing, bounded waits, target-scoped pixel capture, capture-token coordinate recovery, checkpoints, record and replay, dashboard diagnostics, browser discovery, fixture CDP mutation, target-bound fill, focus, and wait operations, accessibility snapshots, history navigation, exact live tab close, sandbox upload and download verification, sandbox restricted copy with checkpoint and hash verification, client integration proposals with atomic JSON apply and undo, asynchronous MCP task execution with cancellation and restart reconciliation, cross platform build validation, release packaging, and dedicated headless Chrome validation are implemented. Native real-Chrome profile acceptance remains pending because this host did not expose the trusted native CUA service.
