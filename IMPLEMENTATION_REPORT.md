...
The `comptrolling@0.1.15` npm package is the pending release for the asynchronous task implementation, with native binaries for Linux x64 and ARM64, macOS x64 and ARM64, and Windows x64 planned by the release workflow. The `v0.1.15` GitHub release is pending workflow completion. The prior `v0.1.14` release is published with 16 verified archives, checksums, SBOMs, and the generated Homebrew formula. Detached signing remains opt-in because no operator signing identity was supplied. The fast verified work is pushed to `feat/fast-verified-execution-v3`; the release tag points to its release commit. GitHub Dependency Graph is disabled for this repository, so dependency review reports a warning while RustSec `cargo-audit` remains enforced by the security workflow.
