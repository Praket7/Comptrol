...
The npm launcher now has an opt in Unix daemon mode. It preserves the external MCP stdio contract, supervises daemon startup, reconnects after socket loss, bounds restart attempts, and does not replay a request that was already sent without a response. The dedicated daemon launcher conformance test passes. Windows continues to use the direct stdio path until named pipe support is complete.
