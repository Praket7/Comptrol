...
The current source passes 47 workspace tests, including deterministic route-plan rationale, verified-trace compilation with redaction and preconditions, stale workflow rejection, unavailable-route rejection, adapter manifest and token validation, and a concurrent delayed-event regression proving EventBus waiters block on notification rather than polling every 5 ms. Browser fixture conformance also covers semantic locators in an open shadow root and a same-origin iframe, while retaining strict ambiguity refusal. The adapter conformance runner smoke-tests four isolated first-party adapter processes.
...
The `v0.1.24` GitHub release workflow completed successfully across all five native targets, published release assets and the npm package, and the package became visible on the public registry after npm processing. The `v0.1.25` workflow is the next release target and adds target-scoped visual evidence for dynamic-site recovery while retaining bounded post-publish registry visibility verification, event-driven history verification, target-list caching, the WSL-versus-Windows binary diagnostic, and isolated adapter runtime validation.
