...
The `comptrol privacy network-endpoints` command reports every optional network route known to this build. Browser CDP is listed only when `COMPTROL_CDP_ENDPOINT` is configured and is used only for an explicit browser action. Remote pairing, update delivery, and telemetry are not implemented or configured.
