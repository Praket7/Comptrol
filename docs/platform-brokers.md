...
The live macOS AX fixture is opt-in in GitHub Actions because it requires a logged-in GUI session with Accessibility permission. Set the repository variable `COMPTROL_RUN_LIVE_MACOS_AX` to `true` on a runner where that permission is available to run it.
